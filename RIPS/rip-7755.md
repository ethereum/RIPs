...
**Include custom exclusivity period for a specified filler**

Let's take the previous example one step further by applying a precheck condition where Address A only wants a specific filler (let's call it Filler A) to be authorized to submit the transaction to the destination chain for a period of time.

In order for this to work, custom exclusivity logic would need to be included in a precheck smart contract (PCSM) deployed to the destination chain. `CrossChainCall` would include the PCSM address in the `precheckContract` field as well as encoded data for exclusivity validation in the `precheckData` field.
...
After Address A calls `requestCrossChainCall` on origin chain, let's say an unintended Filler attempts to submit the transaction on destination chain within the exclusivity period. In this case, the `fulfill` function call will revert because the precheck should fail here. This guarantees that before `expirationTimestamp`, only Filler A can call `fulfill`.
