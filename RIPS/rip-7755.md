...
This proposal introduces a standard for facilitating cross-L2 calls within the Ethereum ecosystem. It aims to minimize trust assumptions by means of a proof system that leverages state sharing between Ethereum and its rollups. This proof system verifies destination chain call execution, enabling secure compensation for off-chain agents that process requested transactions.
...
The system relies exclusively on two onchain verification sources:

1. L2 verification of L1 (Ethereum Mainnet) data:
   - Blockhashes
   - Beacon chain roots
   - _Assumption_: All Ethereum L2s maintain trusted L1 state representations
2. L1 verification of L2 data:
   - Blockhashes
   - State roots
...
- `rewardAsset`: Reward asset address for proof completion, either ERC20 token address or native asset specified in ERC-7528 format.
- `rewardAmount`: Amount of the reward.
- `finalityDelaySeconds`: Minimum timespan after call fulfillment before proof submission is accepted.
...
- Reward amount (0.1001 ETH in this example)
...
Let's introduce a precheck condition where Address A restricts authorization to a specific fulfiller, referred to as Fulfiller A, to submit the transaction to the destination chain for a specified period of time.
...
After properly decoding the `extraData`, the PCSM enforces that only the authorized fulfiller can execute the transaction before the exclusivity period expires. Attempts by other addresses to call `fulfill` during this period will revert.

### Cross-Chain ERC20 Token Transfer

This example explains how to transfer ERC20 tokens (e.g., USDC) between different blockchain networks using RIP7755.
...
> - Requires offchain coordination before execution
...
2. **Outbox Call** \
   Address A submits one call via `CrossChainRequest.calls`:

   ```solidity
   // Transfer from fulfiller to recipient address
   Call({
...
2. **Execution**
   ...
   // USDC moves: Fulfiller -> Address B
   ```
