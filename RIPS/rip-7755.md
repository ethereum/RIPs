...
As seen in the above diagram, the system relies on a few components:

- Contracts for sending / receiving messages
- Offchain fulfillers for relaying messages on behalf of users
- Settlement system for verifying message delivery
...
- `value` (uint256) The native asset value locked during the request transaction
...
1. User submits a UserOp-based request with reward funds to `Outbox`
2. `Outbox` emits discovery event
3. Fulfiller relays the UserOp to destination chain's `EntryPoint` contract
4. `EntryPoint` validates the UserOp and forwards to the Inbox contract for payment
5. Inbox contract acts as a paymaster, verifying and approving the transaction
6. `EntryPoint` executes the UserOp through the user's smart account
7. Upon successful execution, the Inbox records the execution receipt
8. Fulfiller submits proof to `Outbox` to claim the reward
...
The first 20 bytes of `paymasterAndData` represent the Paymaster contract address, followed by a 32-byte validation data field, with the remaining bytes containing the encoded 7755 attributes. The Paymaster contract must also implement the ERC-4337 paymaster interface to validate and pay for UserOps:
...
2. Store the sender and other relevant information for the execution receipt
3. After execution, record the fulfillment information in the Inbox storage
...
The `finalityDelaySeconds` parameter enhances security by ensuring sufficient confidence in destination chain finality before proof submission. This security measure can adapt as L1 and L2 consensus mechanisms evolve.
