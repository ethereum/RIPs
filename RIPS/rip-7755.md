...
abstract contract CrossChainCallOriginator {
  ...
  /// @notice Validates storage proofs and verifies that 
  /// verifyingContractStorageKey on crossChainCall.verifyingContract 
  /// presents a timestamp that's distance from the current destination chain 
  /// block timestamp is greater than crossChainCall.finalityDelaySeconds.
  /// @custom:reverts If storage proof invalid.
  /// @custom:reverts If verifyingContractStorageKey timestamp is less than 
...
contract CrossChainCallFulfillment {
    ...
    mapping(bytes32 fulfillmentHash => uint256 timestamp) public callFulfilledTime;
    ...
    function fulfill(CrossChainCall calldata crossChainCall) external {
        ...
        callFulfilledTime[hash] = block.timestamp;
