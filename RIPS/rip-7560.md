...
- `validationData` - encoded the same as described above, in validateTransaction.
    ...
    - any other value (or revert) means the paymaster rejects this transaction and it is invalid, and can't be included on-chain.
...
A paymaster may also return it, in case it uses some signature (embedded in its paymasterData)
