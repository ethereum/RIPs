...
Note that the `nonce` transaction parameter is defined as a 64-bit integer by the Ethereum protocol for legacy
transaction types,
however in the `TransactionType4` struct it is defined as `uint256 nonce`.
It has a value of `nonceKey << 64 | nonceSequence`.

In RIP-7712 we propose a mechanism that uses the 192 bits `nonceKey` field.
