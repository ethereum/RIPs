...
- Any other returned value is considered to be a failure, and the transaction is rejected.
...
- `MAGIC_VALUE_SIGFAIL` - returned by the paymaster if its paymasterData is expected to contain a signature, but its current value is not valid.
    This value is used during gas estimation, and it helps wallet and apps to determine the gas used by the validation process.
