...
- **authorizer** (20 bytes) - (magic value/address, defined below)
...
Currently, **authorizer** is treated as a magic value:
- `MAGIC_VALUE_SENDER` - returned by the account after it verified the transaction (and agrees to pay for its execution)
- `MAGIC_VALUE_SIGFAIL` - returned by the account if the transaction is syntactically valid, but the signature is not.
    This valid is used during gas estimation, and it helps wallet and apps to determine the gas used by the validation process.
- Any other returned value is considered failure, and the transaction is rejected.
- Future protocol MAY define authorizer as an on-chain contract to perform further account authorization, such as aggregation

A revert by `validateTransaction` indicates rejection of the transaction, and it is not included on-chain.
...
- The `version` parameter is `AA_TX_TYPE`
- The `transaction` parameter is interpreted as an ABI encoding of `TransactionType4`.\
- The `txHash` parameter represents the hash of the AA_TX_TYPE transaction with empty signature, as defined in section
[Calculation of Transaction Type AA_TX_TYPE hash](#calculation-of-transaction-type-aatxtype-hash).
...
The  **authorizer** is treated as a magic value:
- `MAGIC_VALUE_PAYMASTER` - returned by the paymaster after it verified the transaction (and agrees to pay for its execution)
- `MAGIC_VALUE_SIGFAIL` - returned by the paymaster if its paymasterData is expected to contain a signature, but its current value is not valid.
    This valid is used during gas estimation, and it helps wallet and apps to determine the gas used by the validation process.
- Any other returned value is considered failure, and the transaction is rejected, and not included on-chain.
...
The transaction is included in the block even if the execution frame reverts.
...
- `actualGasCost` parameter is the actual amount paid by the paymaster for this transaction (up to this point)
