...
The `deployer` address is invoked with the `deployerData` as call data input.
...
Accepts Transaction Type `AA_TX_TYPE` with fields `validationGasLimit`, `paymasterValidationGasLimit`, `callGasLimit` optional.
...
Returns `{validationGasLimit, paymasterValidationGasLimit, callGasLimit, builderFee}` object.
